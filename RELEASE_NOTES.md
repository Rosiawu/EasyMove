...
- 窗格标签栏使用可透出藤蔓底图的高透毛玻璃，活动窗格以主题最深色独立描边
- 目录列表优先显示，首屏预览与文件夹大小改为受限后台预热；超过 2 秒显示“建立索引中，请稍候”
- 文件夹大小索引持久保存并在文件操作后失效，重启应用后可复用已完成的索引
- 修复后台预览刷新打断拖拽的问题；同盘移动、按键复制到文件夹及外部拖入均通过真实界面测试
...
- 首屏文件夹大小异步返回后一次重排，未完成项稳定排在已知大小之后
